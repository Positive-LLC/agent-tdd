...
- **Codex** (experimental) — `.codex-plugin/plugin.json` (`"skills":"./skills/"`) + `.codex-plugin/marketplace.json`; each entry skill ships `agents/openai.yaml` (`allow_implicit_invocation:false`); invoked via `$atdd`. Codex has no session env hook, so `atdd/SKILL.md` **Step 0** resolves the env. Child agents launch per host through `AGENT_TDD_CLI` — test and impl agents as interactive sessions (`claude` / `opencode` / `codex`, prompt pasted via tmux buffer), the rebase agent headless (`claude -p` / `opencode run` / `codex exec`).
...
- **Project scoping (v1.1.1).** The atdd store is multi-project (per-project DBs + a global `~/.atdd/master.db` registry). The Notes Agent resolves + pins an active project at planning bootstrap (`manifest.json:project_slug`, via `atdd repo where <home_repo>` — first run → `default`, single project → auto, **asks the human only when the home repo is in >1 project**; CORE.md §2). `orch-init.sh` copies the slug into `meta.json`, and `spawn-root.sh` exports it as `$ATDD_PROJECT` on each Root's launch line; the Root's `spawn-test/impl-agent.sh` propagate it onward. Every agent's `atdd` calls inherit `$ATDD_PROJECT` — **agents never pass `--project`**.
