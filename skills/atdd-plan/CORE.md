...
>
> The full design rationale (the "why") lives in
> `docs/atdd-planning-core.md` at the repo root. This file is the "what to do".

> **Path remap note** for skills that source this file (`atdd-fix`, future
> `atdd-feature`): every `${CLAUDE_SKILL_DIR}/...` reference below resolves
> from *your* skill's directory. From `atdd-fix`, treat them as
...
2. **Ensure the manifest:** run
   `bash ${CLAUDE_SKILL_DIR}/recipes/manifest-ensure.sh` and read the JSON it prints. If the
...
3. **Read the NotebookIssue body** (the topology index) and the comment for the active head,
   if any. Use `${CLAUDE_SKILL_DIR}/recipes/notebook-head-get.sh` for the head comment.
4. **Pick the active head** with
   `bash ${CLAUDE_SKILL_DIR}/recipes/topology-next-urgent.sh`. Empty result = no work
...
- **Body (small, slowly-growing):**
  ...
  - Regenerated by `${CLAUDE_SKILL_DIR}/recipes/notebook-index-update.sh` from live GitHub
...
## 7. Recipes (`${CLAUDE_SKILL_DIR}/recipes/`)
