...
1b. **Run the mandatory LSP gate.** Now that the project is pinned and the
   home repo is registered, run `bash ${CLAUDE_SKILL_DIR}/../stack-preflight.sh --repo <home_repo>`
   (the `home_repo` is in the manifest JSON from step 1). It wraps `lsp-surface.sh` and **exits
   non-zero (BLOCKED) while any symbol-precise language has no working LSP** — LSP is mandatory
   (atdd `#32`): `atdd stack verify` reports a `#symbol` anchor as `blocked` without one. Read its
   `missing` array: each entry is a symbol-precise language the repo uses with no working LSP. **Treat
   ...
   project, then `atdd lsp register --repo <home_repo> --lang <lang> --bin <path>`. This is **mandatory** for
   symbol-precise languages — provision, then **re-run `stack-preflight.sh` until it exits 0**
   before planning architecture work (shell / markdown / config are never gated). For a multi-repo
   project, repeat per member repo you actually plan into (use that member's `owner/repo` as
