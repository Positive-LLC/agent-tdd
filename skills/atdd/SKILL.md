...
| `recipes/wave-watcher.sh` | Event-watcher daemon. Cross-platform, in-house: launched via `nohup` + agent waits with a single foreground Bash (internal 10 s poll, 5-min budget per call). Works identically on every host CLI — no `run_in_background` / `bash_bg` needed. See PROTOCOL.md §3.2 step 9. |
